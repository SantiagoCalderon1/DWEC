...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('componentes', function (Blueprint $table) {
            ...
            $table->foreign('idmarca', 'fk_componentes_marcas')->references('id')->on('marcas')->onDelete('restrict');
            $table->string('modelo', 20)->nullable($value = true);
            $table->unsignedBigInteger('idubicacion');
            $table->foreign('idubicacion', 'fk_componentes_ubicacions')->references('id')->on('ubicacions')->onDelete('restrict');
